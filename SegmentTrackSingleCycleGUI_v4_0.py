...
class SistersTool(QtWidgets.QWidget):
    ...
    # def __init__(self, analysis_folder, software_version, raw_data):
    def __init__(self, analysis_folder, software_version):
        QtWidgets.QWidget.__init__(self)
        reload(HandleSistsCoords3D)
        ...
        show_gallery_btn.setFixedSize(100, 25)
    ...
    def put_green_enable(self):
        ...
        if self.put_green_tggl.isChecked():
            self.put_red_tggl.setCheckState(False)
            self.put_gray_tggl.setCheckState(False)
            self.split_spot_tggl.setCheckState(False)

    def put_red_enable(self):
        """Enable the possibility to assign red color to a spot"""
        if self.put_red_tggl.isChecked():
            self.put_gray_tggl.setCheckState(False)
            self.put_green_tggl.setCheckState(False)
            self.split_spot_tggl.setCheckState(False)
    ...
    def click(self, event):
        ...
        if modifiers  ==  QtCore.Qt.ShiftModifier:
            if not self.put_red_tggl.isChecked() and not self.put_green_tggl.isChecked() and not self.split_spot_tggl.isChecked() and not self.put_gray_tggl.isChecked():
                ...
                if self.nuc_tag != 0:
                    ...
                    sgm2show              =  (sgm2show / 3).astype(np.uint8)
                    sgm2show[:, :, :, 2]  +=  (165 * (self.nucs_trck == self.nuc_tag) * (1 - np.sign(self.spts_trck))).astype(np.uint8)
                    sgm2show[:, :, :, 0]  +=  (165 * HandleSistsCoords3D.coords3D2proj(self.sists.sists_3D_coords, s_tags[0])).astype(np.uint8)
                    try:
                        sgm2show[:, :, :, 1] +=  (165 * HandleSistsCoords3D.coords3D2proj(self.sists.sists_3D_coords, s_tags[1])).astype(np.uint8)
                    except IndexError:
                        pass

                elif self.nuc_tag == 0:
                    self.framepp1.clear()
                    sgm2show  =  self.sgmt_raw

                cif  =  self.frame2.currentIndex
                self.frame2.setImage(sgm2show, autoRange=False, autoLevels=False)
                self.frame2.setCurrentIndex(cif)
            ...
            if self.put_red_tggl.isChecked():
                ...
                if self.nuc_tag != 0 and self.spts_left[f_idx, np.round(pos[0]).astype(int), np.round(pos[1]).astype(int)] != 0:
                    # if self.spts_left[f_idx, np.round(pos[0]).astype(int), np.round(pos[1]).astype(int)] != 0:
                    ref_crd                     =  np.where(self.spts_idxs == self.nuc_tag)[0][0]
                    ...
                    self.sists.sists_trck       =  np.copy(bff.sists_trck_fin)
                    self.sists.sists_ints       =  np.copy(bff.sists_ints_fin)
                    self.spts_left              =  SistersIntsWriter.LabelLeft(self.spts_trck, self.sists.sists_trck).spts_left
                    ...
                    self.frame2.setImage(sgm2show, autoRange=False, autoLevels=False)
                    self.frame2.setCurrentIndex(cif)
                    self.frame3.setImage(self.sists.sists_trck, autoRange=False)
                    self.frame3.setCurrentIndex(cif)
            ...
            if self.split_spot_tggl.isChecked():
                ...
                sgm2show   =  np.copy(self.sgmt_raw)
                ref_crd    =  np.where(self.spts_idxs == self.nuc_tag)[0][0]
                if self.nuc_tag != 0 and self.sists.sists_trck[f_idx, np.round(pos[0]).astype(int), np.round(pos[1]).astype(int)] != 0:   # in order to split we need to have at least one spot in the previous or in the followinf frame
                    bff                         =  HandleSistsCoords3D.SplitSpot(ref_crd, self.spts_trck, self.nuc_tag, self.sists.sists_trck, self.sists.sists_3D_coords, self.sists.sists_ints, self.green4D, pos, f_idx, self.pix_size, self.pix_size_Z)
                    self.sists.sists_ints       =  np.copy(bff.sists_ints_fin)
                    self.sists.sists_3D_coords  =  np.copy(bff.sists_3D_coords_fin)
                    self.sists.sists_trck       =  np.copy(bff.sists_trck_fin)
                    self.sists.sists_ints       =  np.copy(bff.sists_ints_fin)
    ...
    def track_sists(self):
        ...
        QtWidgets.QApplication.processEvents()
        try:
            self.sists      =  SisterSplit4.SisterSplit(self.analysis_folder, self.dist_thr)
    ...
    def show_nuc(self):
        ...
        if self.nuc_tag in self.spts_idxs:
            ...
            sgm2show               =  (sgm2show / 3).astype(np.uint8)
            sgm2show[:, :, :, 2]  +=  (165 * (self.nucs_trck == self.nuc_tag) * (1 - np.sign(self.spts_trck))).astype(np.uint8)
            sgm2show[:, :, :, 0]  +=  (165 * (self.sists.sists_trck == s_tags[0])).astype(np.uint8)
            try:
                sgm2show[:, :, :, 1] +=  (165 * (self.sists.sists_trck == s_tags[1])).astype(np.uint8)
            except IndexError:
                pass
    ...
    def save_sists(self):
        ...
        reload(SistersIntsWriter)
        SistersIntsWriter.SistersIntsWriter(self.analysis_folder, self.sists.sists_ints, self.sists.sists_3D_coords, self.spts_idxs, self.dist_thr, self.software_version)

    def two_spts_gallery(self):
        """Generate the gallery of all the splitted time series"""
        reload(TwoSpotsGallery)
